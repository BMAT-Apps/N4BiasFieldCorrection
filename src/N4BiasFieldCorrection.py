...
from bids_N4_bias_field_correction import bids_N4_bias_field_correction, bids_N4_bias_field_correction_docker, find_subjects_and_sessions
import time
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent, add_info):
        ...
        self.setWindowTitle("N4 Bias Field Correction")
        ...
        self.pipeline = "N4BiasFieldCorrection"
        
        sss_slurm = self.add_info.get('sss_slurm')
        
        if sss_slurm == None:
            print('no sss slurm')
            self.tab = N4BiasFieldCorrectionTab(self, sss_slurm)
            layout = QVBoxLayout()
            ...
            self.main_tab = N4BiasFieldCorrectionTab(self, sss_slurm)
...
# TemplateTab
# =============================================================================
class N4BiasFieldCorrectionTab(QWidget):
    """
    """
    ...
    def __init__(self, parent, sss_slurm=None):
        ...
        self.select_sequence_to_N4corr_button = QPushButton("Select image to correct")
        self.select_sequence_to_N4corr_button.clicked.connect(self.select_sequence_to_N4corr)
        self.sequence_to_N4corr_label = QLabel()
        
        self.deriv = QLineEdit(self)
        self.deriv.setPlaceholderText("output derivative folder (default: N4BiasFieldCorrection)")
        
        self.out_name = QLineEdit(self)
        self.out_name.setPlaceholderText("output img name (default: add corr-N4 to input image)")
        ...
        self.button = QPushButton("N4 Bias Field Correction")
        self.button.clicked.connect(self.action)
        
        layout = QGridLayout()
        layout.addWidget(self.select_sequence_to_N4corr_button, 0, 0, 1, 1)
        layout.addWidget(self.sequence_to_N4corr_label, 0, 1, 1, 1)
    ...
    def select_sequence_to_N4corr(self):
        """
        ...
        """
        self.ref_path, _ = QFileDialog.getOpenFileName(self, "Select image to apply N4BiasFieldCorrection", self.bids.root_dir, options=QFileDialog.DontUseNativeDialog)
        if self.ref_path == '':
            return
        ...
        self.sequence_to_N4corr_label.setText(seq_name)
    ...
    def action(self):
        ...
        if self.local:
            ...
            self.is_in_progress(('N4BiasFieldCorrection', True))
            jobs_submitted = []
            ...
            self.is_in_progress(('N4BiasFieldCorrection', False))
    ...
    def run(self):
        ...
        for sub, sess in subjects_and_sessions:
            for ses in sess:
                ...
                if self.use_docker:
                    print('use_docker')
                    bids_N4_bias_field_correction_docker(self.bids, sub, ses, self.ref_path, **kwargs)
                else:
                    bids_N4_bias_field_correction(self.bids, sub, ses, self.ref_path, **kwargs)
