...
def bids_N4_bias_field_correction(bids, sub, ses, ref_path, deriv='N4BiasFieldCorrection', out_name=None):
    ...
    if not out_name:
        ...
        img_d.insert(-1, 'corr-N4')
        out_name = '_'.join(img_d)
        
    subprocess.Popen(f"N4BiasFieldCorrection -i {pjoin(img_p, img_in)} -o {pjoin(sub_ses_deriv, f'{out_name}.nii.gz')}", shell=True).wait()
    
  
def bids_N4_bias_field_correction_docker(bids, sub, ses, ref_path, deriv='N4BiasFieldCorrection', out_name=None):
    ...
    subprocess.Popen(f"docker run --rm --privileged -v {img_p}:/media/input -v {sub_ses_deriv}:/media/output antsx/ants N4BiasFieldCorrection -i {pjoin('/media/input', img_in)} -o {pjoin('/media/output', f'{out_name}.nii.gz')}", shell=True).wait()
...
if __name__ == '__main__':
    ...
    description = '''
bids_N4BiasFieldCorrection:
    Compute N4BiasFieldCorrection from ANTs  
    '''
    ...
    parser.add_argument('--derivative', '-d', dest='deriv', type=str, help='derivative folder name to store the output (default: N4BiasFieldCorrection)', default='N4BiasFieldCorrection', required=False)
    parser.add_argument('--out-name', '-o', dest='out_name', type=str, help='name of the output sequence name (default: add corr-N4 to input img name)', default=None, required=False)
    ...
    for sub, sess in subjects_and_sessions:
        for ses in sess:
            ...
            if args.use_docker:
                bids_N4_bias_field_correction_docker(bids, sub, ses, args.ref_path, deriv=args.deriv, out_name=args.out_name)
            else:
                bids_N4_bias_field_correction(bids, sub, ses, args.ref_path, deriv=args.deriv, out_name=args.out_name)
